...
namespace PracticalShooterApp.ViewModels
{
    ...
    [QueryProperty(nameof(SearchRuleNumeric), nameof(SearchRuleNumeric))]
    public class RulesPageViewModel : BaseViewModel
    {
        private string _inlineIdentifier;
        private string _searchedRuleNumeric;
        ...
        private IGlossaryService _glossaryService => DependencyService.Get<IGlossaryService>();
        ...
        public string SearchRuleNumeric
        {
            get => _searchedRuleNumeric;
            set => SetProperty(ref _searchedRuleNumeric, value);
        }
        ...
        private async void LoadRules(int sectionId)
        {
            ...
            foreach (var rule in rules)
            {
                var ruleModel = new RuleModel()
                {
                    ...
                    IsNotSubRule = true,
                    IsSearched = rule.Numeric == _searchedRuleNumeric,
            ...
            // For Glossary
            if (SectionName == "Glossary")
            {
                var glossaries = await _glossaryService.GetGlossariesFromCurrentRulebook();
