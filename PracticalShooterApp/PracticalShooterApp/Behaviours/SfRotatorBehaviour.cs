...
namespace PracticalShooterApp.Behaviours
{
    public class SfRotatorBehaviour : Behavior<SfRotator>
    {
        #region Fields
        ...
        public static async void StartAnimation(List<View> childElement, BoardingContentModel item)
        {
            if (childElement != null && item != null)
            {
                ...
                animation.Commit((item as BoardingContentModel).RotatorView as ContentView, "animation", 16, 500);
        ...
        public void Animation(SfRotator rotator, double selectedIndex)
        {
            if (rotator != null && rotator.ItemsSource != null && rotator.ItemsSource.Any())
            {
                ...
                if (Device.RuntimePlatform != Device.UWP)
                {
                    ...
                    var childElement = (((currentItem as BoardingContentModel).RotatorView as ContentView).Children[0] as StackLayout).Children.ToList();
                    if (childElement != null && childElement.Count > 0)
                    {
                        StartAnimation(childElement, currentItem as BoardingContentModel);
                    }
                    ...
                    if (index != this.previousIndex)
                    {
                        ...
                        var previousChildElement = (((previousItem as BoardingContentModel).RotatorView as ContentView).Children[0] as StackLayout).Children.ToList();
